...
These scripts also solves potential issues of Fcitx5 on Firefox or Firefox ESR from Mozilla Team PPA. For Firefox and Thunderbird, either [Switch from Snap Firefox and Thunderbird to Deb Firefox and Thunderbird from Mozilla Team PPA](#switch-from-snap-firefox-and-thunderbird-to-deb-firefox-and-thunderbird-from-mozilla-team-ppa) or [Switch from Snap Firefox and Thunderbird to Deb Firefox ESR and Thunderbird from Mozilla Team PPA](#switch-from-snap-firefox-and-thunderbird-to-deb-firefox-esr-and-thunderbird-from-mozilla-team-ppa) is recommended. For Linux Mint user, you may want to install from their [official repository](http://packages.linuxmint.com) instead.

After switching Firefox to Deb, if the `Close Firefox: Firefox is already running, but is not responding. To use Firefox, you must first close the existing Firefox process, restart your device, or use a different profile.` error pop-up shows, run `sudo systemctl restart firefox-apparmor` to solve it.
